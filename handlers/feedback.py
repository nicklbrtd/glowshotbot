...
def _build_main_text(ideas: list[dict]) -> str:
    header = (
        ...
        "Предложи улучшение, фичу или изменение. \n"
        "За интересные и реально полезные идеи можно получить премиум.\n\n"
    )

    if not ideas:
        return header + "Предложи идею через кнопку ниже."
...
async def feedback_new(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        "✍️ Напиши свою идею <b>одним</b> сообщением.\n"
        "Можно прикрепить <b>одно</b> вложение.\n"
    )
