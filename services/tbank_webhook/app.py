...
def parse_order_id(order_id: str):
    """
    Рекомендованный формат, который тебе ОЧЕНЬ упростит жизнь:
    GS_<tgid>_<plan>_<ts>
    plan: w/m/q (week/month/3months)
    пример: GS_123456789_m_170343
    """
    try:
...
async def tbank_notify(req: Request):
    ...
    # Если ключей нет — сервис поднят, но он НЕ ГОТОВ принимать реальные платежи
    ...
    try:
        if body.get("Amount") is not None:
            amount_rub = int(body.get("Amount")) // 100  # Amount обычно в копейках
    except Exception:
        amount_rub = None

        log.info("notify: status=%s success=%s order_id=%s payment_id=%s", status, success, order_id, payment_id)

    # Премиум выдаём на CONFIRMED
    if success and status == "CONFIRMED":
        parsed = parse_order_id(order_id)
        if not parsed:
            # не ломаем вебхук, просто логируем
